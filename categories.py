class PremiumBackHoodies:
    def __init__(self):
        self.products = [
        '"Bad Vibes" - Graphic Hoodie Men and Women',
        '"Good Vibes" - Graphic Hoodie Men and Women',
        'NOT HERE FOR YOUR APPROVAL: Graphie Hoodie for Men and Women',
        '"Kindness" - Graphic Hoodie Men and Women',
        '"Dekeing" - Graphic Hoodie Men and Women',
        'CAT: Graphic Hoodie for Men and Women',
        '"Selfcare" - Graphic Hoodie Men and Women',
        'Graphic Hoodie for Men and Women - "You Problem"',
        '"Ran Out of Coffee" - Graphic Hoodie Men and Women',
        'Keep it simple: Graphic Hoodie For Men and Women',
        '"Basketball Love" - We The North Edition : Graphic Hoodie for Men and Women',
        '"You Problem - Devil" - Graphic Hoodie Men and Women'
    ]
        self.price = 63.99
        self.compare_at_price = 89.98

class PremiumFrontHoodies:
    def __init__(self):
        self.products = [
            'MOOD: Graphic Hoodie For Men and Women',
            '1834: Graphic Hoodie for Men and Women'
        ]
        self.price = 55.99
        self.compare_at_price = 79.98

class PremiumFrontSweatshirts:
    def __init__(self):
        self.products = [
            '"MOOD": Sweatshirt for Men and Women',
            '"1834": Sweatshirt for Men and Women'
        ]
        self.price = 52.99
        self.compare_at_price = 75.98

class PremiumBackSweatshirts:
    def __init__(self):
        self.products = [
            'Keep it simple: Sweatshirt for Men and Women',
            '"Good Vibes" - Graphic Crewneck Sweatshirt Women and Men',
            '"Kindness" - Graphic Crewneck Sweatshirt Women and Men',
            '"Basketball Love" - We The North Edition : Crewneck Sweatshirt for Men and Women'
        ]
        self.price = 58.99
        self.compare_at_price = 83.98

class SignatureToronnoHoodies:
    def __init__(self):
        self.products = [
            '"Toronno": Graphic Hoodie For Men and Women',
            '"TORCC. EST 2022": Graphic Hoodie For Men and Women',
            '"ToronnoWithTCN": Graphic Hoodie For Men and Women',
            '"CnTower Toronno": Graphic Hoodie For Men and Women',
            '"SlotToronno": Graphic Hoodie For Men and Women',
        ]
        self.price = 48.99
        self.compare_at_price = 57.99

class SignatureToronnoSweatshirt:
    def __init__(self):
        self.products = [
            '"CnTower Toronno": Sweatshirt for Men and Women',
            '"Circle T with CN Tower": Sweatshirt for Men and Women',
            '"SlotToronno": Sweatshirt For Men and Women',
            '"Toronno": Sweatshirt For Men and Women',
            '"ToronnoWithTCN": Sweatshirt For Men and Women',
            '"TORCC. EST 2022": Sweatshirt For Men and Women'
        ]
        self.price = 44.99
        self.compare_at_price = 53.99

class SignatureWTNHoodies:
    def __init__(self):
        self.products = [
            '"North-CN" - We The North Edition : Graphic Hoodie for Men and Women',
            '"Flying Hoop" - We The North Edition : Graphic Hoodie for Men and Women',
            '"WTN" - We The North Edition : Graphic Hoodie for Men and Women',
            '"#wethenorth" - We The North Edition : Graphic Hoodie for Men and Women',
            '"Wings" We The North Edition : Graphic Hoodie for Men and Women'
        ]
        self.price = 55.99
        self.compare_at_price = 79.98

class SignatureWTNSweatshirts:
    def __init__(self):
        self.products = [
            '"Flying Hoop" - We The North Edition : Crewneck Sweatshirt for Men and Women',
            '"WTN" - We The North Edition : Crewneck Sweatshirt for Men and Women',
            '"North-CN" - We The North Edition : Crewneck Sweatshirt for Men and Women',
            '"#wethenorth" - We The North Edition : Crewneck Sweatshirt for Men and Women',
            '"Wings" - We The North Edition : Crewneck Sweatshirt for Men and Women'
        ]
        self.price = 52.99
        self.compare_at_price = 75.98

class ClassicSweatshirt:
    def __init__(self):
        self.products = [
            'Classic Sweatshirt'
        ]
        self.price = 41.99
        self.compare_at_price = 59.98

class ClassicHoodie:
    def __init__(self):
        self.products = [
            'Classic Hoodie'
        ]
        self.price = 45.99
        self.compare_at_price = 63.98

class TorccCollectionHoodies:
    def __init__(self):
        self.products = [
            '"TORCC": Graphic Hoodie for Men',
            '"TORCC": Graphic Hoodie for Women',
            '"Inspire": Graphic Hoodie For Women',
            '"Inspire": Graphic Hoodie For Men',
            '"Fighter": Graphic Hoodie For Women',
            '"Fighter": Graphic Hoodie For Men',
            '"Hope": Graphic Hoodie For Women',
            '"Hope": Graphic Hoodie For Men',
            '"Believe": Graphic Hoodie For Women',
            '"Believe": Graphic Hoodie For Men',
            '"Hustle": Graphic Hoodie For Women',
            '"Hustle": Graphic Hoodie For Men',
            '"Dreamer": Graphic Hoodie For Women',
            '"Dreamer": Graphic Hoodie For Men'
        ]
        self.price = 63.99
        self.compare_at_price = 89.98

class TorccCollectionSweatshirts:
    def __init__(self):
        self.products = [
            '"Dreamer": Sweatshirt for women',
            '"Dreamer": Sweatshirt For Men',
            '"Fighter": Sweatshirt for Men',
            '"Fighter": Sweatshirt for Women',
            '"Believe": Sweatshirt for women',
            '"Believe": Sweatshirt for men',
            '"Hope": Sweatshirt for Women',
            '"Hope": Sweatshirt For Men',
            '"Hustle" : Sweatshirt for Women',
            '"Inspire" : Sweatshirt for Women',
            '"Inspire" : Sweatshirt for Men',
            '"Hustle": Sweatshirt for men'
        ]
        self.price = 55.99
        self.compare_at_price = 79.98

class NewSweatshirts: 
    def __init__(self):
        self.products = [
            'Weekend : Sweatshirt For Men And Women',
            f"\"Damn That's So 90'S\" : Sweatshirt For Men And Women",
            '"Have A Nice Trip": Sweatshirt For Men And Women',
            f"\"Keep It Rollin'\" : Sweatshirt For Men And Women",
            f"\"Growth'\" : Graphic Sweatshirt For Men And Women",
            '"Dystopia" : Graphic Sweatshirt For Men And Women',
            '"LOVE" Sweatshirt For Men And Women : Ranbir Kapoor Sweatshirt In #TJMM'
            '"Out Of This World": Sweatshirt For Men And Women',
            'Wish You Were Weird : Sweatshirt For Men And Women'
        ]
        self.price = 55.99
        self.compare_at_price = 79.98

class NewHoodies:
    def __init__(self):
        self.products = [
            '"Nobody Cares": Graphic Hoodie for Men',
            '"Keep It Rolling": Graphic Hoodie for Men',
            '"Have a nice trip": Graphic Hoodie for Men',
            f"\"Damn - That's so 90s\": Graphic Hoodie for Men",
            '"Nobody Cares": Graphic Hoodie for Women',
            f'"Have a nice trip": Graphic Hoodie for Women',
            f"\"Damn - That's so 90s\": Graphic Hoodie for Women",
            '"Keep It Rolling": Graphic Hoodie for Women',
            '"Growth": Graphic Hoodie for Women'
        ]
        self.price = 63.99
        self.compare_at_price = 89.98

class Oversized:
    def __init__(self):
        self.products = [
            'Oversized'
        ]
        self.price = 24.99
        self.compare_at_price = 24.99

class Sweatshirt:
    def __init__(self):
        self.products = [
            'Sweatshirt'
        ]
        self.price = 39.99
        self.compare_at_price = 79.98

class Hoodie:
    def __init__(self):
        self.products = [
            'Hoodie'
        ]
        self.price = 44.99
        self.compare_at_price = 89.98

class ClassicTees:
    def __init__(self):
        self.products = [
            'Graphic T-Shirt'
        ]
        self.price = 19.99
        self.compare_at_price = 19.99
